...
public class SigatureUtil {
	...
	public static String getSign(String userName) {
		...
		try {
			String privStr ="-----BEGIN PRIVATE KEY-----\n"
					+ "MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgWBzF3AuSv2oSCRjC\n"
					+ "9JrtZZg3KkQzDT9zEdv6ZvoWKYWhRANCAATW4VyX+GivTN95MnWilYFi80Vig20d\n"
					+ "tnNkPMfbJUKmY2f2jv7RhvePsebXHHEr8wdzwqyzinmOAQAvsNS79/4Q\n"
					+ "-----END PRIVATE KEY-----";

			String pubStr = "-----BEGIN PUBLIC KEY-----\n"
					+ "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE1uFcl/hor0zfeTJ1opWBYvNFYoNt\n"
					+ "HbZzZDzH2yVCpmNn9o7+0Yb3j7Hm1xxxK/MHc8Kss4p5jgEAL7DUu/f+EA==\n"
					+ "-----END PUBLIC KEY-----";
			// generate signature
			GenTLSSignatureResult result = GenTLSSignatureEx(1400099628,
					userName, privStr);
			sign = result.urlSig;
			if (0 == result.urlSig.length()) {
				System.out.println("GenTLSSignatureEx failed: "
						+ result.errMessage);
				return sign;
			}
			...
			// check signature
			// CheckTLSSignatureResult checkResult = CheckTLSSignatureEx(
			// result.urlSig, 1400000955, "xiaojun", pubStr);
			CheckTLSSignatureResult checkResult = CheckTLSSignatureEx(
					result.urlSig, 1400099628, userName, pubStr);
			if (checkResult.verifyResult == false) {
				System.out.println("CheckTLSSignature failed: "
						+ result.errMessage);
				return null;
			}

			System.out.println("\n---\ncheck sig ok -- expire time "
					+ checkResult.expireTime + " -- init time "
	...
	public static GenTLSSignatureResult GenTLSSignature(long expire,
			String strAppid3rd, long skdAppid, String identifier,
			long accountType, String privStr) throws IOException {
		...
		parser.close();
		PrivateKey privKeyStruct = converter
				.getPrivateKey((PrivateKeyInfo) obj);

		// Create Json string and serialization String
		String jsonString = "{" + "\"TLS.account_type\":\"" + accountType
		...
		try {
			...
			String userSig = new String(base64_url.base64EncodeUrl(Arrays
					.copyOfRange(compressBytes, 0, compressBytesLength)));
	...
	public static CheckTLSSignatureResult CheckTLSSignature(String urlSig,
			String strAppid3rd, long skdAppid, String identifier,
			long accountType, String publicKey) throws Exception {
		...
		byte[] compressBytes = base64_url.base64DecodeUrl(urlSig
		...
		String jsonString = new String(Arrays.copyOfRange(decompressBytes, 0,
				decompressLength));
		...
		try {
			...
			if (System.currentTimeMillis() / 1000 - Long.parseLong(sigTime) > Long
					.parseLong(sigExpire)) {
			...
			Object obj = parser.readObject();
			parser.close();
			PublicKey pubKeyStruct = converter
	 ...
	 * @throws IOException
	 */
	public static GenTLSSignatureResult GenTLSSignatureEx(long skdAppid,
			String identifier, String privStr) throws IOException {
	 ...
	 */
	public static GenTLSSignatureResult GenTLSSignatureEx(long skdAppid,
			String identifier, String privStr, long expire) throws IOException {
	...
	public static CheckTLSSignatureResult CheckTLSSignatureEx(String urlSig,
			long sdkAppid, String identifier, String publicKey)
			throws Exception {
		...
		try {
			...
			if (Integer.parseInt(strSdkAppid) != sdkAppid) {
				result.errMessage = new String("sdkappid " + strSdkAppid
						+ " in tls sig not equal sdkappid " + sdkAppid
						+ " in request");
				return result;
			}
